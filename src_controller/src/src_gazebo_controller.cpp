...
class SRCGazeboController : public rclcpp::Node {
private:
  bool verbose = false;
...
public:
  SRCGazeboController() : Node("src_gazebo_controller") {
    ...
    cmd_vel_sub = create_subscription<Twist>(
      "cmd_vel", 10,
      std::bind(&SRCGazeboController::cmd_vel_callback, this, std::placeholders::_1)
    );
...
int main(int argc, char **argv) {
  ...
  auto src_gazebo_controller = std::make_shared<SRCGazeboController>();

  rclcpp::spin(src_gazebo_controller);
  rclcpp::shutdown();
